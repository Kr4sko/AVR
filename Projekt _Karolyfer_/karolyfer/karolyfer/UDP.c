...
const __flash char UDP_Create_UDP[]		= "AT+CIPSTART=4,\"UDP\",\"192.168.1.51\",1112,8081,0\r\n";
...
const __flash char UDP_WiFi[]	=	"AT+CWJAP_CUR=\"Buba_G-2.4GHz\",\"alamakota2018\"\r\n";
...
void UDP_delay()
{
	// funkcja opozniajaca na podstawie danych otrzymancytwh od modulu
...
//Extracts received data from UDP transmission
void UDP_receive_data()
{
	/*
	//Archaic version, ineffective and faulty
	char *data = strstr(resp, ":");
	//creating substring from correct received response, data is from ':' to the end of the string
	return data;
	*/
	
	//Possibly better method utilizing known characteristics of ESP8266 module
	
	int i = 0;
	while(Module_Response[i] != ":") i++;
	for(int j = 0; i < strlen(Module_Response); j++)
	{
		if(j < 20) Received_Data[j] = Module_Response[++i];
		else break;
	}
}
